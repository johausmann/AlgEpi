...
* DESeq2 tries to fit a trend line to capture behaviour of dispersion
...
* Count reads in different windows of the genome
...
* Visualising the correlation (e.g. using Deeptools plotCorrelation)

* Second step to check if IP has worked correctly (IP efficiency)

* Count reads in bins over the genome for input and ChIP, then sort and normalize it
...
* Scroll along the genome in bins

* Call significant windows using Poisson distribution to distinguish between background noise (to calculate p-values)

* Merge significant windows to call peaks

* Read shifting conducted as follows:

* Usually a peak from the positive and a peak from the negative strand would be recieved (left and right from the position, where the protein is sitting)

* Fragment size is estimated (by distance of the two peaks or from paired end information)

* MACS2 shifts the peaks towards the centre (half of the fragment length)
