...
* Target: quantify differences in DNA methylation between 2 sample groups based
...
* ATAC-Seq
  ...
  + very sensitive (less cells needed than for DNaseI-Seq experiment, nowadays even single-cell)
...
* Quantitative assesment of __GpCs__ and __CpGs__ &rarr; two types of information
...
* Gaussian mixture to cluster windows per chromosome
  ...
  + Learn mixture model to get clusters &rarr; probability that window belong to cluster
...
* Fragments of different length, when sequencing &rarr; e.g. one ot two nucleosomes
  + Short fragments in center (open chromatin region)

* Model as HMM the three states Center (open), Background (no open chromatin) and Nucleosome (adjavent to center)
...
* Signals in bins alogn genome &rarr; correlation plot
...
* If domain of TF is known, that does not mean the motiv is also known
...
2. Normalize the counts in the matrix

* Probability can be calculated easily (multiply the normalized values for each symbol at each position)

* Problem: if character not given in alignment probability becomes always 0
...
* Positions in the binding site model are independent is a strong simplification (will have a impact on the 3D shape)
...
* SLIM (Sparse local inhomogeneous mixture model): Mixture of conditional contributions for different previous and current position
  ...
  + Inhomogeneous, because the probabilities for each position can be different
...
* ATAC-seq smaller preferences than in TF binding motifs (max arround 0.3 bit) but preference is given
  + palindromic
  + because two transposase molecules are involved in the cutting
  + large sequence bias (because of two molecules)
...
* For all assays preferences in the minor groove with at and arround the cutting/methylation position were found

### What is computational footprinting?

* Dips in peaks of open-chromatin data &rarr; TF bound to DNA (hinders DNase to cut DNA)

* Predicted footprint regions are a good starting point for binding motif search

### How does the HINT method or footprinting work?
...
* Seperate signal to forward and reverse

* HINT has several states for forward and reverse to treat them separately (Two transposase molecules)
